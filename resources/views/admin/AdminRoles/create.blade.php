            ...
            <div class="col-md-7 col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">
                            @if (isset($role))
                                {{ __('locale.role.update_role') }}
                            @else
                                {{ __('locale.role.create_role') }} @endif
                        </h4>
                    </div>
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical"
                                @if (isset($role)) action="{{ route('admin.roles.update', $role->uid) }}" @else action="{{ route('admin.roles.store') }}" @endif
                                method="post">
                                @if (isset($role))
                                    {{ method_field('PUT') }}
                                @endif
                                        ...
                                        @foreach ($permissions as $category)
                                            <div class="divider divider-start divider-info mt-4">
                                                <div class="divider-text text-uppercase fw-bold text-primary">
                                                    {{ __('locale.menu.' . $category['title']) }}</div>
                                            </div>
